...
SDL_Surface* SDLCommonFunc::LoadImage(std::string file_path)
{
	...
	if (load_image != NULL)
	{
		...
		// thay doi mau nen nhan vat ve mau nen cua game
...
{
    g_img_menu = LoadImage("menugame1.png");
    if (g_img_menu == NULL)
    {
        return 1;
    }
    ...
    bool seclected[kMenuItemNum] = { 0,0 };
...
int SDLCommonFunc::ShowGameOver(SDL_Surface* des, TTF_Font* font,std::string mark_)
{
    g_img_menu = LoadImage("gameover.png");
    if (g_img_menu == NULL)
    {
        return 1;
    }
    ...
    text_menu[0].SetText("mark: " + mark_);
